...
function find_restaurant_by_id($id) {
    $conn = db_connect();
    $sql = "SELECT * FROM resturant where RestaurantID=$id limit 1";
    $result = $conn->query($sql);
    //var_dump($result);die;
    $conn->close();
    if ($result->num_rows > 0) {
...
function new_restaurant( $email,$restaurantname, $ownername, $phone, $password,  $address, $target,  $create_date ) {
    $conn = db_connect();
    $stmt = $conn->prepare("INSERT INTO resturant (Email,RestaurantName,OwnerName,PhoneNumber,Password,Address, Image_Url,Create_date) values(?,?,?,?,?,?,?,? )");
$stmt->bind_param('sssssssi',$email,$restaurantname,$restaurantname, $phone, $password,  $address,$target ,$create_date );
    $result = $stmt->execute();
    if ($result) {
        $stmt->close();
        $conn->close();
...
function new_contact($email,$name,$comment,$sent_date){
$conn=db_connect();
$stmt=$conn->prepare("INSERT INTO tbcontact(Email,Name,Comment,date)values(?,?,?,?)");
$stmt->bind_param('sssi',$email,$name,$comment,$sent_date );
$result = $stmt->execute();
    if ($result) {
        $stmt->close();
        $conn->close();
...
function restaurant_login($email, $password) {

    $conn = db_connect();

    $sql = "SELECT RestaurantID,Email FROM resturant where Email='$email' and Password='$password' limit 1";
    $result = $conn->query($sql);
    $conn->close();
    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
